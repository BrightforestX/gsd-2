...
const TOOLBAR = [
  ...
  { id: 'linkml',   icon: '◈',  label: 'LinkML',   title: 'GSD capabilities schema map (React Flow) — toggle off to return to instance editor' },
...
export function NodePalette({
  ...
  const instanceToolsDisabled = Boolean(linkmlSchemaMode);

  const actions = {
    home:     onHome,
    load:     () => { if (!instanceToolsDisabled) fileInputRef.current?.click(); },
    json:     () => { if (!instanceToolsDisabled) onSaveJson(); },
    yaml:     () => { if (!instanceToolsDisabled) onSaveYaml(); },
    linkml:   onLinkmlSchemaToggle,
    settings: onSettings,
  };
    ...
    return (
      <aside className="np-sidebar np-sidebar--collapsed">
        ...
        <div className="np-icon-strip">
          {TOOLBAR.map(t => {
            const dis = (t.id === 'load' || t.id === 'json' || t.id === 'yaml') && instanceToolsDisabled;
            return (
            <button
              key={t.id}
              className={`np-icon-only${t.id === 'linkml' && linkmlSchemaMode ? ' np-icon-only--active' : ''}`}
              onClick={actions[t.id]}
              title={dis ? 'Exit LinkML schema mode to use this action' : t.title}
              disabled={dis}
            >
              {t.icon}
            </button>
  ...
  return (
    <aside className="np-sidebar">
      ...
      <div className="np-toolbar">
        <div className="np-toolbar__btns">
          {TOOLBAR.map(t => {
            ...
            <button
              key={t.id}
              className={`np-toolbar__btn${t.id === 'linkml' && linkmlSchemaMode ? ' np-toolbar__btn--active' : ''}`}
              onClick={actions[t.id]}
              title={dis ? 'Exit LinkML schema mode to use this action' : t.title}
              disabled={dis}
            >
      ...
      {/* ── Classes (instance editor) / schema hint (LinkML map) ── */}
      <div className="np-section">
        {linkmlSchemaMode ? (
          <div className="np-schema-hint">
            <strong>GSD capabilities</strong>
            <p>
              Canvas shows <code>gsd-capabilities.linkml.yaml</code> (classes, <code>is_a</code>, slot ranges).
              Updates when you save the file. Click <strong>LinkML</strong> again to return to the instance
              editor.
            </p>
          </div>
        ) : (
